...
export default class MobComponentManager {
    private static readonly MaxDistanceToPlayer = 120;
    ...
    static register(entity: Entity, preExisting = false, isWithinRange?: boolean) {
        ...
        // dont stall players or nightfuries (since only 1 nightfury can exist at a time)
        if (!isWithinRange && entity.typeId !== "minecraft:player" && entity.typeId !== "gm1_zen:nightfury") {
    ...
    static forceUnstallEntity(entity: Entity) {
        const entry = [...this.stalledEntities].find((entry) => entry.entity.id === entity.id);
        if (entry) {
            this.stalledEntities.delete(entry);
            this.register(entity, false, true);
        }
    }
    ...
    static countRegisteredEntities() {
        const count = {};
        for (const entityId in this.mobComponents) {
            const typeId = this.mobComponents[entityId][0]?.entity.typeId;

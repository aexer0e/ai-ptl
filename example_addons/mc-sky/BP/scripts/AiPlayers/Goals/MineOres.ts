...
export default class extends Goal {
    ...
    onEnter() {
        GameData.events.emit("aiPlayerMessage", this.entity, EntityStore.get(this.entity, "name"), "MineOre", 1);
        this.equipTool();
    ...
    setAndGetTargetBlock() {
        const blockData = EntityStore.temporary.get(this.entity, "rcBlockCache").ore;
    ...
    findBlockToMine() {
        ...
        for (const blockLocation of blockLocations) {
            ...
            if (GameData.ItemGroup["minecraft:ore"].includes(block!.typeId)) {
                return block;
            }
        }
    }
    ...
    equipTool() {
        ...
        const allTools = GameData.ItemGroup["minecraft:pickaxe"];

...
export default class V3 {
    ...
    static getBlocksInVolume(volume: Volume): V3[] {
        const result = [] as V3[];
        for (let x = volume.min.x; x <= volume.max.x; x++) {
            for (let y = volume.min.y; y <= volume.max.y; y++) {
                for (let z = volume.min.z; z <= volume.max.z; z++) {
                    result.push(new V3(x, y, z));
                }
            }
        }
        return result;
    ...
    static distanceFlat(a: Vector3, b: Vector3): number {
        return new V3(a).setY(0).subtract(b.x, 0, b.z).length();
    }

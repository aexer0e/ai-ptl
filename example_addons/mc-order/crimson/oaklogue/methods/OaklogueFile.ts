...
export default class OaklogueFile {
    ...
    public addToLang(text: string, address: string, langspace: string) {
        ...
        for (let i = 0; i < textByLine.length; i++) {
            ...
            const key = `oaklogue.${hashWithLangspace}.${i}`;
            this.lang.push(`${key}=${line}`);

            // If line isn't empty, push it to the output
            if (line.length > 0) output.push({ translate: key });

            // Push new line
            output.push({ text: "\n" });
        }

        // Return output with last new line removed
    ...
    private processTextScene(sceneTag: string, scene: TextScene) {
        ...
        for (let i = 0; i < options.length; i++) {
            ...
            const button: VanillaButton = {
                name: this.ensureRawText(text, `${sceneTag}.button.${i}`, langspace),

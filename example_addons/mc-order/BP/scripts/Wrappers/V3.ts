...
export default class V3 {
    ...
    asYawPitch() {
        const yaw = Math.atan2(this.z, this.x) * (180 / Math.PI);
    ...
    static asBlockLocationId(x: number, y: number, z: number): number {
        const id = (Math.floor(x) * 1205 + Math.floor(y) * 641225 + Math.floor(z)) * 546;
        return id;
    }
    ...
    static getBlocksInRadius(location: Vector3, range: number): V3[] {
        const v3 = new V3(location);
        const result = [] as V3[];
        for (let x = -range; x <= range; x++) {
            for (let y = -range; y <= range; y++) {
                for (let z = -range; z <= range; z++) {
                    if (Math.sqrt(x * x + y * y + z * z) <= range) {
                        result.push(v3.add(x, y, z));
                    }
                }
            }
        }
    ...
    static getBlocksInSquare(location: Vector3, radius: number): V3[] {
        const v3 = new V3(location);
        const result = [] as V3[];
        for (let x = -radius; x <= radius; x++) {
            for (let z = -radius; z <= radius; z++) {
                result.push(v3.add(x, 0, z));
            }
        }
        return result;
    }
    ...
    applyOffset(offsetRanges: Vector3): V3 {
    ...
    addRandomness(range: number): V3 {
        const x = this.x + MathUtil.random(-range, range);
        const y = this.y + MathUtil.random(-range, range);
        const z = this.z + MathUtil.random(-range, range);
        return new V3(x, y, z);
    }
    ...
    clampY(min: number, max: number) {
        return new V3(this.x, Math.min(max, Math.max(min, this.y)), this.z);
    }
    ...
    // Normalize the vectory by only changing the x and z values
    normalizeXZOnly() {
        const lengthXZ = Math.sqrt(this.x * this.x + this.z * this.z);

        if (lengthXZ === 0) {
            return new V3(0, this.y === 0 ? 0 : this.y / Math.abs(this.y), 0).clean();
        }

        if (Math.abs(this.y) > 1) {
            return new V3(0, this.y / Math.abs(this.y), 0).clean();
        }

        const scale = Math.sqrt(1 - this.y * this.y) / lengthXZ;

        const newX = this.x * scale;
        const newZ = this.z * scale;

        return new V3(newX, this.y, newZ).clean();
    }

    static locationInView(location: Vector3, direction: Vector3, point: Vector3) {
        const directionToBlock = new V3(point).subtractV3(location);
        const dot = directionToBlock.dot(direction);
        return dot >= 0;
    }

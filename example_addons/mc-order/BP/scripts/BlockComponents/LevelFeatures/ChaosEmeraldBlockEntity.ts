...
export default class ChaosEmeraldBlockEntity extends AbstractBlockComponent {
    public static get identifier() {
        return "gm1_ord:chaos_emerald_entity";
    }
    ...
    private static onPlace(event: BlockComponentOnPlaceEvent) {
        {
            ...
            const hasAlreadyRegistered = event.block.permutation.getState("gm1_ord:chaos_emerald_entity" as keyof BlockStateSuperset) as
                | boolean
                | undefined;
            ...
            const chaosEmeraldEntity = EntityUtil.spawnEntity(ChaosEmeraldType, blockLocation, dimension);
            // We have to cast our custom states to the list of of vanilla block states to avoid a type error. TODO: Remove when possible
            block.setPermutation(block.permutation.withState("gm1_ord:chaos_emerald_entity" as keyof BlockStateSuperset, true));
            EntityStore.set(chaosEmeraldEntity, "blockLocation", blockLocation);
        }
    }
    ...
    private static onPlayerDestroy(event: BlockComponentPlayerDestroyEvent) {
        {
            ...
            ChaosEmeraldEntities.forEach((entity) => entity.remove());
        }
    }
}
